...
describe("LicensesRepository", () => {
  ...
  describe("getLicensesResult", () => {
    ...
    it("should throw PluginResultNotAvailable when no result exists", async () => {
      ...
      await expect(
        repository.getLicensesResult("non-existent-analysis"),
    ...
    it("should throw PluginFailed when analysis status is Failure", async () => {
      ...
      await expect(
        repository.getLicensesResult("analysis-123"),
    ...
    it("should handle complex licenses output structure", async () => {
      ...
      expect(result.workspaces["frontend"]!.LicensesDepMap["MIT"]).toContain(
        "react@18.0.0",
      );
      expect(
        result.workspaces["backend"]!.LicenseComplianceViolations,
      ).toHaveLength(0);
    });
