...
describe("SbomUtilsService", () => {
  ...
  const createMockSBOMOutput = (
    overrides: Partial<SBOMOutput> = {},
  ): SBOMOutput => ({
    workspaces: {
      [mockWorkspace]: {
        dependencies: {
  ...
  describe("getDependencyData", () => {
    ...
    it("should return dependency details with vulnerability information", async () => {
      ...
      expect(vulnerabilitiesUtilsService.getVulnsResult).toHaveBeenCalledWith(
        mockAnalysisId,
      );
    });
  ...
  describe("service instantiation", () => {
    ...
    it("should have correct dependencies injected", () => {
      ...
      expect(service["vulnerabilitiesUtilsService"]).toBe(
