...
export class CleanupIncorrectTables1754760000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TABLE IF EXISTS "policy_analyses_analysis" CASCADE`,
    );
    await queryRunner.query(
      `DROP TABLE IF EXISTS "organization_projects_project" CASCADE`,
    );
    await queryRunner.query(
      `DROP TABLE IF EXISTS "organization_policies_policy" CASCADE`,
    );
    await queryRunner.query(
      `DROP TABLE IF EXISTS "organization_integrations_integration" CASCADE`,
    );
    await queryRunner.query(
      `DROP TABLE IF EXISTS "organization_notifications_notification" CASCADE`,
    );
    await queryRunner.query(
      `DROP TABLE IF EXISTS "organization_owners_user" CASCADE`,
    );
    await queryRunner.query(
      `DROP TABLE IF EXISTS "user_integrations_integration" CASCADE`,
    );
    ...
    await queryRunner.query(
      `DROP TYPE IF EXISTS "public"."analysis_schedule_type_enum"`,
    );
  ...
  public async down(_queryRunner: QueryRunner): Promise<void> {
    ...
    throw new Error(
      "This migration cannot be reverted. Restore from backup if needed.",
    );
  }
}
