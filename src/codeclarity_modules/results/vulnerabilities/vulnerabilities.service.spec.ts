...
describe("VulnerabilitiesService", () => {
  ...
  beforeEach(async () => {
    ...
    findingsUtilsService = module.get<VulnerabilitiesUtilsService>(
  ...
  describe("getStats", () => {
    it("should return vulnerability analysis stats", async () => {
      ...
      mockAnalysisResultsService.checkAccess.mockResolvedValue(undefined);
      mockFindingsUtilsService.getVulnsResult.mockResolvedValue(
        mockVulnsOutputWithArray,
      );
  ...
  describe("getVulnerabilities", () => {
    ...
    it("should handle vulnerability merging with multiple affected dependencies", async () => {
      const mockFindingsWithDuplicates = [
        {
          ...
          Weaknesses: [],
          OSVMatch: {
            Vulnerability: { Vlai_score: 0.8, Vlai_confidence: 0.9 },
          },
          NVDMatch: {
            Vulnerability: { Vlai_score: 0.7, Vlai_confidence: 0.8 },
          },
          Conflict: false,
        },
        {
      ...
      expect(
        (result.data as { Affected: unknown[]; VLAI: unknown[] }[])[0]!
          .Affected,
      ).toHaveLength(2);
      expect(
    ...
    it("should handle CVE vulnerability descriptions", async () => {
      ...
      expect(mockNVDRepository.getVulnWithoutFailing).toHaveBeenCalledWith(
        "CVE-2024-1234",
      );
      expect(
        mockOSVRepository.getVulnByCVEIDWithoutFailing,
      ).toHaveBeenCalledWith("CVE-2024-1234");
    ...
    it("should handle GHSA vulnerability descriptions", async () => {
      ...
      expect(
        mockOSVRepository.getVulnByOSVIDWithoutFailing,
      ).toHaveBeenCalledWith("GHSA-xxxx-yyyy-zzzz");
      expect(
        (result.data as { Description: string }[])[0]!.Description,
      ).toContain("Test GHSA summary");
    });
    ...
    it("should handle weaknesses with CWE information", async () => {
      ...
      mockFindingsUtilsService.getFindingsData.mockResolvedValue([
        mockFindingWithWeakness,
      ]);
      mockFindingsFilterService.filter.mockReturnValue([
        [mockMergedFindingWithWeakness],
        {},
      ]);
      mockFindingsSortService.sort.mockReturnValue([
        mockMergedFindingWithWeakness,
