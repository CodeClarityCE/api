...
describe("LicensePolicyService", () => {
  ...
  describe("get", () => {
    ...
    it("should throw error when policy not found", async () => {
      ...
      // Act & Assert
      await expect(
        service.get("org-123", "non-existent", mockUser),
      ).rejects.toThrow("EntityNotFound");
    });
    ...
    it("should require USER role for read access", async () => {
      ...
      // Act & Assert
      await expect(
        service.get("org-123", "policy-123", mockUser),
      ).rejects.toThrow(unauthorizedError);
      expect(membershipsRepository.hasRequiredRole).toHaveBeenCalledWith(
        "org-123",
        "user-123",
  ...
  describe("update", () => {
    it("should throw not implemented error", async () => {
      ...
      // Act & Assert
      await expect(
        service.update("org-123", "policy-123", {}, mockUser),
  ...
  describe("remove", () => {
    it("should throw not implemented error", async () => {
      ...
      // Act & Assert
      await expect(
        service.remove("org-123", "policy-123", mockUser),
      ).rejects.toThrow("Method not implemented.");
      expect(membershipsRepository.hasRequiredRole).toHaveBeenCalledWith(
        "org-123",
        "user-123",
