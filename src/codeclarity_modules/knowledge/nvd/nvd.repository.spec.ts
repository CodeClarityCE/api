...
describe("NVDRepository", () => {
  ...
  describe("getVuln", () => {
    ...
    it("should throw EntityNotFound when vulnerability is not found", async () => {
      ...
      await expect(
        nvdRepository.getVuln("CVE-2023-NONEXISTENT"),
  ...
  describe("getVulnWithoutFailing", () => {
    it("should return NVD vulnerability when found", async () => {
      ...
      const result =
        await nvdRepository.getVulnWithoutFailing("CVE-2021-44228");
    ...
    it("should handle repository errors", async () => {
      ...
      await expect(
        nvdRepository.getVulnWithoutFailing("CVE-2021-44228"),
  ...
  describe("Real-world vulnerability scenarios", () => {
    ...
    it("should handle vulnerability with multiple weaknesses", async () => {
      ...
      const weaknesses = result.weaknesses as {
    ...
    it("should handle vulnerability with complex affected products", async () => {
      ...
      const affectedFlattened = result.affectedFlattened as Record<
        string,
        string[]
      >;
      expect(Object.keys(affectedFlattened)).toHaveLength(2);
      expect(affectedFlattened["apache:log4j"]).toEqual([
        "2.0-beta9",
        "2.15.0",
      ]);
    });
  ...
  describe("Concurrent operations", () => {
    ...
    it("should handle mixed success and failure scenarios", async () => {
      ...
      const nonExistentResults = await Promise.all(
        nonExistentCVEs.map((cveId) =>
          nvdRepository.getVulnWithoutFailing(cveId),
        ),
      );
