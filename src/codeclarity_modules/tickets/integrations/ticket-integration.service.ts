...
export class TicketIntegrationService {
  ...
  getProvider(name: ExternalTicketProvider): ITicketIntegrationProvider {
    ...
    if (!provider) {
      throw new BadRequestException(
        `Integration provider ${name} is not available`,
      );
    }
    return provider;
  }
  ...
  async deleteIntegrationConfig(
    ...
    if (!config) {
      throw new NotFoundException(
        `Integration config for ${provider} not found`,
      );
    }

    await this.configRepository.remove(config);
    this.logger.log(
      `Deleted ${provider} integration config for org ${organizationId}`,
    );
  }
  ...
  async refreshOAuthToken(
    ...
    const tokens = await providerInstance.refreshToken(
      clickUpConfig.refresh_token,
    );
    ...
    if (tokens.expires_in) {
      clickUpConfig.token_expiry = new Date(
  ...
  async getWorkspaces(
    ...
    if (!providerInstance.getWorkspaces) {
      throw new BadRequestException(
        `Provider ${provider} does not support workspaces`,
      );
    }
  ...
  async getSpaces(
    ...
    if (!providerInstance.getSpaces) {
      throw new BadRequestException(
        `Provider ${provider} does not support spaces`,
      );
    }
  ...
  async getFolders(
    ...
    if (!providerInstance.getFolders) {
      throw new BadRequestException(
        `Provider ${provider} does not support folders`,
      );
    }
  ...
  async getLists(
    ...
    if (!providerInstance.getLists) {
      throw new BadRequestException(
        `Provider ${provider} does not support lists`,
      );
    }
  ...
  async createSpace(
    ...
    if (!providerInstance.createSpace) {
      throw new BadRequestException(
        `Provider ${provider} does not support creating spaces`,
      );
    }
  ...
  async createFolder(
    ...
    if (!providerInstance.createFolder) {
      throw new BadRequestException(
        `Provider ${provider} does not support creating folders`,
      );
    }
  ...
  async createList(
    ...
    if (!providerInstance.createList) {
      throw new BadRequestException(
        `Provider ${provider} does not support creating lists`,
      );
    }
  ...
  async syncTicketToExternal(
    ...
    const existingLink = ticket.external_links?.find(
    ...
    const providerInstance = this.getProvider(provider);
    const result = await providerInstance.createExternalTicket(
      ticket,
      config.config,
    );
    ...
    this.logger.log(
      `Synced ticket ${ticketId} to ${provider}: ${result.external_id}`,
    );
  ...
  async updateExternalTicket(ticketId: string, linkId: string): Promise<void> {
    ...
    if (!externalLink.sync_enabled) {
      this.logger.warn(
        `Sync disabled for link ${linkId}, skipping external update`,
      );
      return;
    }
    ...
    if (!config?.enabled) {
      this.logger.warn(
        `${externalLink.provider} integration disabled, skipping update`,
      );
      return;
    }
  ...
  async updateExternalTicketStatus(
    ...
    for (const link of ticket.external_links || []) {
      ...
      try {
        ...
        this.logger.log(
          `Updated external ticket ${link.external_id} status to ${status}`,
        );
      } catch (error) {
        this.logger.error(
  ...
  async syncFromExternal(
    ...
    const { status: externalStatus, externalStatus: rawExternalStatus } =
      await (
        provider as {
