...
export class GithubAuthController {
  ...
  async githubAuthFinalize(
    ...
    const token: GithubOAuthAccessTokenResponse = await this.getToken(
      oauth2FinalizeBody.code,
    );
    ...
    // (5) Return jwt token
    return {
      data: await this.authService.authenticateGithubSocial(authenticatedUser),
    };
  }
