...
export class VulnerabilitiesSortService {
  sort(
    ...
    if (
      sortBy === null ||
      sortBy === undefined ||
      !ALLOWED_SORT_BY.includes(sortBy)
    ) {
    ...
    if (sortBySafe === "severity") {
      sorted = vulnerabilities.sort(
        (a: VulnerabilityMerged, b: VulnerabilityMerged) => {
    ...
    // }
      ...
      sorted = vulnerabilities.sort(
        (a: VulnerabilityMerged, b: VulnerabilityMerged) => {
          if (
            (a.Severity?.Exploitability ?? 0.0) >
            (b.Severity?.Exploitability ?? 0.0)
          )
            return sortDirectionSafe === "DESC" ? -1 : 1;
          if (
            (a.Severity?.Exploitability ?? 0.0) <
      ...
      sorted = vulnerabilities.sort(
        (a: VulnerabilityMerged, b: VulnerabilityMerged) => {
          ...
          const aVal =
            typeof aRaw === "string" || typeof aRaw === "number" ? aRaw : "";
          const bVal =
            typeof bRaw === "string" || typeof bRaw === "number" ? bRaw : "";
