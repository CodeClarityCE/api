...
import { Analysis, AnalysisStatus } from 'src/base_modules/analyses/analysis.entity';
...
export class TicketAutomationService {
    ...
    async processCompletedAnalysis(
        ...
        this.logger.log(`Processing completed analysis ${analysisId} for auto-resolve`);
        ...
        const currentVulnIds = await this.getVulnerabilityIdsFromAnalysis(analysisId);
        this.logger.log(`Found ${currentVulnIds.length} vulnerabilities in analysis ${analysisId}`);
    ...
    async findTicketsToResolve(
        ...
        const ticketsToResolve = openTickets.filter(
            (ticket) => ticket.vulnerability_id && !currentVulnIds.includes(ticket.vulnerability_id)
        );
    ...
    private async resolveFixedVulnerabilities(
        ...
        const ticketsToResolve = await this.findTicketsToResolve(orgId, projectId, currentVulnIds);
     ...
     */
    private async getVulnerabilityIdsFromAnalysis(analysisId: string): Promise<string[]> {
        ...
        if (!result) {
            this.logger.log(`No vulnerability result found for analysis ${analysisId}`);
            return [];
        }
    ...
    async processRecentlyCompletedAnalyses(since: Date): Promise<number> {
        ...
        const recentAnalyses = completedAnalyses.filter((a) => a.ended_on && a.ended_on >= since);
