...
describe('CWERepository', () => {
    ...
    const mockCWE: CWE = {
        ...
        related_weaknesses: [{ nature: 'ChildOf', cwe_id: 'CWE-20', view_id: '1000' }],
        modes_of_introduction: [{ phase: 'Implementation', note: 'Failure to validate input' }],
        common_consequences: [
            {
        ...
        alternate_terms: [
            { term: 'XSS', description: 'Commonly used abbreviation for Cross-Site Scripting' }
        ],
    ...
    describe('getCWE', () => {
        ...
        it('should throw EntityNotFound when CWE is not found', async () => {
            ...
            await expect(cweRepository.getCWE('CWE-999')).rejects.toThrow(EntityNotFound);
            expect(mockRepository.findOne).toHaveBeenCalledWith({
                where: {
                    cwe_id: 'CWE-999'
                }
        ...
        it('should handle different CWE ID formats', async () => {
            const testCases = ['CWE-1', 'CWE-10', 'CWE-100', 'CWE-1000', 'CWE-79', 'CWE-89'];
    ...
    describe('getCWEWithoutFailing', () => {
        ...
        it('should handle repository errors', async () => {
            ...
            await expect(cweRepository.getCWEWithoutFailing('CWE-79')).rejects.toThrow(
                'Database connection failed'
            );
        });
    ...
    describe('Integration scenarios', () => {
        it('should handle concurrent requests for the same CWE', async () => {
            ...
            const promises = Array.from({ length: 5 }, () => cweRepository.getCWE('CWE-79'));
        ...
        it('should handle mixed success and failure scenarios', async () => {
            ...
            const nonExistentResults = await Promise.all(
                nonExistentCWEs.map((cweId) => cweRepository.getCWEWithoutFailing(cweId))
            );
