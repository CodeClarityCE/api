...
describe("IntegrationsService", () => {
  ...
  describe("getVCSIntegrations", () => {
    it("should retrieve VCS integrations successfully", async () => {
      ...
      jest
        .spyOn(integrationsRepository, "getVCSIntegrations")
        .mockResolvedValue(mockPaginatedIntegrations);

      // Act
      const result = await service.getVCSIntegrations(
        "org-123",
        paginationConfig,
        mockUser,
      );
  ...
  describe("getIntegration", () => {
    it("should retrieve integration successfully", async () => {
      // Arrange
      jest
        .spyOn(organizationsRepository, "doesIntegrationBelongToOrg")
        .mockResolvedValue(true);
      jest.spyOn(membershipsRepository, "hasRequiredRole").mockResolvedValue();
      jest
        .spyOn(integrationsRepository, "getIntegrationById")
      ...
      ).toHaveBeenCalledWith("integration-123", "org-123");
      expect(membershipsRepository.hasRequiredRole).toHaveBeenCalledWith(
        "org-123",
        "user-123",
        MemberRole.USER,
    ...
    it("should throw NotAuthorized when integration does not belong to org", async () => {
      ...
      expect(membershipsRepository.hasRequiredRole).not.toHaveBeenCalled();
  ...
  describe("removeIntegration", () => {
    ...
    it("should handle NotAMember error by converting to NotAuthorized", async () => {
      ...
      jest
        .spyOn(membershipsRepository, "hasRequiredRole")
        .mockRejectedValue(notAMemberError);
  ...
  describe("markIntegrationAsInvalid", () => {
    it("should throw not implemented error", async () => {
      // Act & Assert
      await expect(
        service.markIntegrationAsInvalid("integration-123"),
      ).rejects.toThrow("Method not implemented.");
    });
