...
describe("Dashboard Utility Functions", () => {
  ...
  describe("getProjectScoreClassFromScore", () => {
    ...
    it("should return B+ for scores >= 0.1 and < 0.25", () => {
      ...
      expect(getProjectScoreClassFromScore(0.15)).toBe(
        ProjectGradeClass.B_PLUS,
      );
      expect(getProjectScoreClassFromScore(0.24)).toBe(
        ProjectGradeClass.B_PLUS,
      );
    });
    ...
    it("should return C+ for scores >= 0.4 and < 0.55", () => {
      ...
      expect(getProjectScoreClassFromScore(0.45)).toBe(
        ProjectGradeClass.C_PLUS,
      );
      expect(getProjectScoreClassFromScore(0.54)).toBe(
        ProjectGradeClass.C_PLUS,
      );
    });
    ...
    it("should return D+ for scores >= 0.7 and < 0.85", () => {
      ...
      expect(getProjectScoreClassFromScore(0.75)).toBe(
        ProjectGradeClass.D_PLUS,
      );
      expect(getProjectScoreClassFromScore(0.84)).toBe(
        ProjectGradeClass.D_PLUS,
      );
    });
    ...
    it("should handle floating point precision", () => {
      ...
      expect(getProjectScoreClassFromScore(0.1000000001)).toBe(
        ProjectGradeClass.B_PLUS,
      );
      expect(getProjectScoreClassFromScore(0.2499999999)).toBe(
        ProjectGradeClass.B_PLUS,
      );
    });
  });
  ...
  describe("edge cases and error handling", () => {
    ...
    it("should handle extreme score values", () => {
      expect(getProjectScoreClassFromScore(Number.MAX_VALUE)).toBe(
        ProjectGradeClass.D,
      );
      expect(getProjectScoreClassFromScore(Number.MIN_VALUE)).toBe(
        ProjectGradeClass.A,
      );
