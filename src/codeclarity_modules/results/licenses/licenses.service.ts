...
export class LicensesService {
    ...
    for (const [licenseId, depsUsingLicense] of Object.entries(
      ...
      const licenseInfo: LicenseInfo = {
        ...
        license_compliance_violation:
          licensesWorkspaceInfo.LicenseComplianceViolations?.includes(
            licenseId,
          ) ?? false,
        unable_to_infer:
      ...
      try {
        const licenseData =
          await this.licenseRepository.getLicenseData(licenseId);
        ...
        if (licenseData.details.licenseProperties) {
          licenseInfo.license_properties =
            licenseData.details.licenseProperties;
        }
        licenseInfo.references = licenseData.details.seeAlso;
      } catch (error) {
        console.error(
          `Failed to get license data for license ID: ${licenseId}`,
          error,
        );
  ...
  async getStatus(
    ...
    const licensesOutput =
      await this.licensesUtilsService.getLicensesResult(analysisId);

    if (licensesOutput.analysis_info.private_errors.length) {
      return {
        public_errors: licensesOutput.analysis_info
          .public_errors as StatusError[],
        private_errors: licensesOutput.analysis_info
          .private_errors as StatusError[],
  ...
  private async getDependencyVersions(
    ...
    console.warn(
      "getDependencyVersions not implemented - returning empty versions",
    );
