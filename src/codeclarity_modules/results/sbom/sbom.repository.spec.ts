...
describe("SBOMRepository", () => {
  ...
  describe("getSbomResult", () => {
    ...
    it("should throw PluginResultNotAvailable when no result exists", async () => {
      ...
      await expect(
        repository.getSbomResult("non-existent-analysis"),
    ...
    it("should handle complex SBOM structure with multiple workspaces", async () => {
      ...
      expect(
        (result.workspaces as any).frontend.dependencies.react["18.0.0"].name,
      ).toBe("react");
      expect(
        (result.workspaces as any).backend.dependencies.express["4.18.0"].name,
      ).toBe("express");
    });
