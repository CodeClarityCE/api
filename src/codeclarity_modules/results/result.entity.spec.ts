...
describe("Result Entity", () => {
  ...
  it("should handle vulnerability scan results", () => {
    ...
    expect(
      (result.result.workspaces as any).default.affected_vulnerabilities,
    ).toBeDefined();
    expect(
      (result.result.workspaces as any).default.Vulnerabilities,
    ).toHaveLength(1);
  });
  ...
  it("should handle license scan results", () => {
    ...
    expect(
      (result.result.workspaces as any).default.LicensesDepMap,
    ).toBeDefined();
    expect(
      (result.result.workspaces as any).default.LicensesDepMap.MIT,
    ).toHaveLength(2);
  });
