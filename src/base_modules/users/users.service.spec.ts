...
describe('UsersService', () => {
    ...
    describe('getUser', () => {
        it('should return user when userId matches authenticated user', async () => {
            ...
            const result = await service.getUser('test-user-id', mockAuthenticatedUser);
    ...
    describe('setDefaultOrg', () => {
        it('should set default organization successfully', async () => {
            ...
            jest.spyOn(organizationsRepository, 'saveOrganization').mockResolvedValue(
                orgWithDefault
            );
            ...
            expect(organizationsRepository.saveOrganization).toHaveBeenCalledWith(orgWithDefault);
        });
        ...
        it('should throw when user does not have required role', async () => {
            ...
            await expect(
                service.setDefaultOrg('test-user-id', 'test-org-id', mockAuthenticatedUser)
            ).rejects.toThrow(NotAuthorized);
        });
    });
    ...
    describe('register', () => {
        it('should register user successfully', async () => {
            ...
            jest.spyOn(organizationsRepository, 'saveOrganization')
                .mockResolvedValueOnce(createdOrg)
                .mockResolvedValueOnce(createdOrg);

            // Mock the saveUser method to return a user with an ID
            jest.spyOn(usersRepository, 'saveUser').mockImplementation((user: any) => {
                user.id = 'new-user-id';
        ...
        it('should throw PasswordsDoNotMatch when passwords do not match', async () => {
            ...
            await expect(service.register(invalidUserData)).rejects.toThrow(PasswordsDoNotMatch);
        });
    ...
    describe('sendUserRegistrationVerificationEmail', () => {
        ...
        it('should send registration verification email when user not verified', async () => {
            ...
            jest.spyOn(emailService, 'sendRegistrationConfirmation').mockResolvedValue(undefined);
        ...
        it('should return early if user is already verified', async () => {
            ...
            jest.spyOn(usersRepository, 'getUserByEmail').mockResolvedValue(verifiedUser);
        ...
        it('should throw FailedToSendAccountRegistrationVerificationEmail when email sending fails', async () => {
            ...
            jest.spyOn(emailRepository, 'getMailByType').mockResolvedValue(null);
            jest.spyOn(emailRepository, 'saveMail').mockResolvedValue(undefined);
            jest.spyOn(emailService, 'sendRegistrationConfirmation').mockRejectedValue(
                new Error('Email failed')
            );
    ...
    describe('confirmRegistration', () => {
        ...
        it('should confirm registration successfully', async () => {
            ...
            jest.spyOn(emailRepository, 'getActivationMail').mockResolvedValue(mailWithUser);
            jest.spyOn(usersRepository, 'saveUser').mockResolvedValue(mailWithUser.user);
        ...
        it('should throw AccountRegistrationVerificationTokenInvalidOrExpired when mail not found', async () => {
            jest.spyOn(emailRepository, 'getActivationMail').mockResolvedValue(null as any);

            await expect(service.confirmRegistration('token', 'userIdHash')).rejects.toThrow(
                AccountRegistrationVerificationTokenInvalidOrExpired
            );
        });
    });
    ...
    describe('updatePassword', () => {
        it('should update password successfully', async () => {
            ...
            jest.spyOn(usersRepository, 'getUserById').mockResolvedValue(userWithOldPassword);
            jest.spyOn(authService, 'validateCredentials').mockResolvedValue([
                true,
                userWithOldPassword
            ]);
            jest.spyOn(authService, 'hashPassword').mockResolvedValue('new-hashed-password');
            jest.spyOn(usersRepository, 'saveUser').mockResolvedValue(userWithOldPassword);
            ...
            expect(usersRepository.saveUser).toHaveBeenCalledWith(userWithOldPassword);
        });
        ...
        it('should throw PasswordsDoNotMatch when passwords do not match', async () => {
            ...
            await expect(
                service.updatePassword('test-user-id', invalidPasswordData, mockAuthenticatedUser)
    ...
    describe('updatePersonalInfo', () => {
        it('should update personal info successfully', async () => {
            ...
            jest.spyOn(usersRepository, 'getUserById').mockResolvedValue(userToUpdate);
    ...
    describe('delete', () => {
        it('should delete user successfully with correct password', async () => {
            ...
            jest.spyOn(usersRepository, 'getUserById').mockResolvedValue(userToDelete);
            jest.spyOn(authService, 'validateCredentials').mockResolvedValue([true, userToDelete]);
            jest.spyOn(usersRepository, 'deleteUser').mockResolvedValue(undefined);

            await service.delete('test-user-id', mockAuthenticatedUser, 'correct-password');
        ...
        it('should throw NotAuthorized when password is missing for non-social user', async () => {
            ...
            await expect(service.delete('test-user-id', mockAuthenticatedUser)).rejects.toThrow(
                NotAuthorized
            );
        });
        ...
        it('should throw NotAuthorized when password is incorrect', async () => {
            const userToDelete = { ...mockUser, social: false };
            jest.spyOn(usersRepository, 'getUserById').mockResolvedValue(userToDelete);
            jest.spyOn(authService, 'validateCredentials').mockResolvedValue([false, userToDelete]);
    ...
    describe('existsSocialUser', () => {
        it('should throw not implemented error', async () => {
            await expect(service.existsSocialUser('socialId', 'GITHUB' as any)).rejects.toThrow(
                'Method not implemented.'
            );
        });
    });
