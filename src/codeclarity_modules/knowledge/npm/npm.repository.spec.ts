...
describe("NPMPackageRepository", () => {
  ...
  describe("getNpmPackageData", () => {
    ...
    it("should throw EntityNotFound when package is not found", async () => {
      ...
      await expect(
        npmRepository.getNpmPackageData("nonexistent-package"),
    ...
    it("should handle empty string package name", async () => {
      ...
      await expect(npmRepository.getNpmPackageData("")).rejects.toThrow(
        EntityNotFound,
      );
      expect(mockRepository.findOne).toHaveBeenCalledWith({
        where: { name: "" },
    ...
    it("should handle special characters in package name", async () => {
      ...
      await expect(
        npmRepository.getNpmPackageData(specialPackageName),
      ).rejects.toThrow(EntityNotFound);
      expect(mockRepository.findOne).toHaveBeenCalledWith({
    ...
    it("should handle very long package names", async () => {
      ...
      await expect(
        npmRepository.getNpmPackageData(longPackageName),
      ).rejects.toThrow(EntityNotFound);
      expect(mockRepository.findOne).toHaveBeenCalledWith({
  ...
  describe("Edge cases and error handling", () => {
    it("should handle undefined package name gracefully", async () => {
      ...
      await expect(
        npmRepository.getNpmPackageData(undefined as any),
      ).rejects.toThrow(EntityNotFound);
      expect(mockRepository.findOne).toHaveBeenCalledWith({
    ...
    it("should handle null package name gracefully", async () => {
      ...
      await expect(
        npmRepository.getNpmPackageData(null as any),
      ).rejects.toThrow(EntityNotFound);
      expect(mockRepository.findOne).toHaveBeenCalledWith({
        where: { name: null },
      });
