...
jest.mock("@nestjs/common", () => {
  const MockValidationPipe = jest.fn().mockImplementation(function (
    this: any,
    options,
  ) {
  ...
  const MockClassSerializerInterceptor = jest.fn().mockImplementation(function (
    this: any,
  ) {
...
jest.mock("./interceptors/ResponseBodyInterceptor", () => {
  const MockResponseBodyInterceptor = jest.fn().mockImplementation(function (
    this: any,
  ) {
    this.intercept = jest.fn();
    return this;
...
jest.mock("./types/error.types", () => {
  const MockValidationFailed = jest.fn().mockImplementation(function (
    this: any,
    errors,
  ) {
...
describe("Main.ts Bootstrap Configuration and Testing", () => {
  ...
  describe("Error Scenarios and Edge Cases", () => {
    ...
    it("should test mock application creation failure", async () => {
      ...
      await expect(mockCreate()).rejects.toThrow(
