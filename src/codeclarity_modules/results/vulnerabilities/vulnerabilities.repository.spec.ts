...
describe("VulnerabilitiesRepository", () => {
  ...
  beforeEach(async () => {
    ...
    repository = module.get<VulnerabilitiesRepository>(
      VulnerabilitiesRepository,
    );
  ...
  describe("getVulnsResult", () => {
    ...
    it("should throw PluginResultNotAvailable when no result exists", async () => {
      ...
      await expect(
        repository.getVulnsResult("non-existent-analysis"),
    ...
    it("should handle complex vulnerabilities structure with multiple workspaces", async () => {
      ...
      expect(
        (result.workspaces as any).frontend.affected_vulnerabilities[
          "CVE-2024-1111"
        ].severity,
      ).toBe("CRITICAL");
      expect(
        (result.workspaces as any).backend.affected_vulnerabilities[
          "CVE-2024-2222"
        ].severity,
      ).toBe("LOW");
    });
    ...
    it("should handle vulnerabilities with no affected vulnerabilities", async () => {
      ...
      expect(
        Object.keys(
          (result.workspaces as any).default.affected_vulnerabilities,
        ),
      ).toHaveLength(0);
      expect((result.workspaces as any).default.Vulnerabilities).toHaveLength(
        0,
      );
    });
