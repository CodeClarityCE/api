...
export class ClickUpService implements ITicketIntegrationProvider {
    ...
    async validateConfig(config: IntegrationConfig): Promise<boolean> {
        ...
        if (clickUpConfig.auth_method === ClickUpAuthMethod.API_KEY) {
            if (!clickUpConfig.api_key) {
                throw new BadRequestException('api_key is required for API Key authentication');
            }
        } else if (clickUpConfig.auth_method === ClickUpAuthMethod.OAUTH) {
            if (!clickUpConfig.access_token) {
                throw new BadRequestException('access_token is required for OAuth authentication');
            }
        }
    ...
    async exchangeCodeForToken(code: string, _redirectUri: string): Promise<OAuthTokenResponse> {
    ...
    async getWorkspaces(config: IntegrationConfig): Promise<ExternalWorkspace[]> {
        const clickUpConfig = config as ClickUpConfig;
        const response = await this.makeRequest<ClickUpTeamsResponse>('/team', clickUpConfig);
    ...
    async getSpaces(workspaceId: string, config: IntegrationConfig): Promise<ExternalSpace[]> {
        const clickUpConfig = config as ClickUpConfig;
    ...
    async getFolders(spaceId: string, config: IntegrationConfig): Promise<ExternalFolder[]> {
        const clickUpConfig = config as ClickUpConfig;
    ...
    async getLists(parentId: string, config: IntegrationConfig): Promise<ExternalList[]> {
        const clickUpConfig = config as ClickUpConfig;
    ...
    async createExternalTicket(
        ...
        const priority = this.mapPriority(ticket.priority, clickUpConfig.priority_mapping);
    ...
    async updateExternalTicketStatus(
        ...
        this.logger.log(`Updated ClickUp task ${externalId} status to: ${clickUpStatus}`);
    }

    async deleteExternalTicket(externalId: string, config: IntegrationConfig): Promise<void> {
        const clickUpConfig = config as ClickUpConfig;
    ...
    private async makeRequest<T>(
        ...
        if (!response.ok) {
            ...
            throw new BadRequestException(`ClickUp API error (${response.status}): ${errorText}`);
        }
    ...
    private mapPriority(priority: TicketPriority, customMapping?: Record<string, number>): number {
