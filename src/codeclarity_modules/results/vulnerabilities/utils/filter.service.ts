...
export class VulnerabilitiesFilterService {
    filter(
        ...
        function filterBySearchKey(vulnerabilities: VulnerabilityMerged[]): VulnerabilityMerged[] {
            ...
            for (const vulnerability of vulnerabilities) {
                for (const affected of vulnerability.Affected) {
                    if (affected.AffectedDependency?.toLocaleLowerCase().includes(searchKeyLower)) {
                        toReturn.push(vulnerability);
                        continue;
                    }
                }
                if (vulnerability.Vulnerability?.toLocaleLowerCase().includes(searchKeyLower)) {
                    toReturn.push(vulnerability);
                    continue;
                }
            }
        ...
        function hasOwaspId(vulnerability: VulnerabilityMerged, owaspId: string): boolean {
            return vulnerability.Weaknesses?.some((w) => w.OWASPTop10Id === owaspId) ?? false;
        }

        /** Check if vulnerability has any categorized OWASP weakness */
        function hasAnyCategorizedOwasp(vulnerability: VulnerabilityMerged): boolean {
            return vulnerability.Weaknesses?.some((w) => w.OWASPTop10Id !== '') ?? false;
        }
        ...
        function passesOwaspFilter(vulnerability: VulnerabilityMerged, filters: string[]): boolean {
            ...
            for (const [filterName, owaspId] of Object.entries(OWASP_FILTER_MAP)) {
                if (filters.includes(filterName) && !hasOwaspId(vulnerability, owaspId)) {
                    return false;
                }
            }
            // Check uncategorized filter (exclude if has any categorized OWASP)
            if (filters.includes('owasp_uncategorized') && hasAnyCategorizedOwasp(vulnerability)) {
                return false;
            }
            return true;
        }
