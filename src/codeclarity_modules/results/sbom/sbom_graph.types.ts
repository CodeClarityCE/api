...
export class GraphTraversalUtils {
   ...
   */
  static findMinimalPathsToTarget(
    nodeId: string,
    graph: GraphDependency[],
  ): GraphDependency[] {
    const result: GraphDependency[] = [];
    ...
    for (const parentId of targetNode.parentIds) {
      ...
      if (parent) {
        ...
        if (!currentPath.some((node) => node.id === parent.id)) {
          ...
          const pathsFromParent = this.findAllPathsToTarget(
            parent,
            nodeMap,
            newPath,
          );
