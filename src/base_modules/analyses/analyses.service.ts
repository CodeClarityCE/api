...
export class AnalysesService {
  ...
  async create(
    ...
    const analyzer = await this.analyzersRepository.getAnalyzerById(
    ...
    if (analysisData.languages && analysisData.languages.length > 0) {
      ...
      const supportedRequestedLanguages = analysisData.languages.filter(
        (lang) => analyzer.supported_languages.includes(lang),
      );
    ...
    const config_structure: Record<
      string,
      Record<string, AnalyzerStepConfigOption>
    > = {};
      ...
      for (const step of stage) {
        ...
        if (step.config) {
          for (const [key, value] of Object.entries(step.config)) {
            ...
            config_structure[step.name]![key] =
              value as AnalyzerStepConfigOption;
          }
        }
      }
    ...
    for (const [pluginName, plugin_config] of Object.entries(
      config_structure,
    )) {
  ...
  async getChart(
    ...
    const vulnOutput: VulnsOuptut =
      await this.vulnerabilitiesRepository.getVulnsResult(id);

    // Fetch licenses output for the specified analysis ID
    const licensesOutput: LicensesOutput =
      await this.licensesRepository.getLicensesResult(id);
  ...
  async getMany(
    ...
    // (2) Check if the project belongs to the org
    await this.projectsRepository.doesProjectBelongToOrg(
      projectId,
      organizationId,
    );
  ...
  async createScheduledExecution(originalAnalysisId: string): Promise<string> {
    ...
    const originalAnalysis = await this.analysesRepository.getAnalysisById(
      originalAnalysisId,
      {
        analyzer: true,
    ...
    const savedAnalysis =
      await this.analysesRepository.saveAnalysis(newAnalysis);
  ...
  private groupResultsByDay(results: AnalysisResultRecord[]): AnalysisRun[] {
    ...
    const grouped = results.reduce<Record<string, AnalysisResultRecord[]>>(
      (acc, result) => {
