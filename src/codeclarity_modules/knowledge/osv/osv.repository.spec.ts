...
describe("OSVRepository", () => {
    ...
    affected: [
      {
        ...
        database_specific: {
          source:
            "https://github.com/github/advisory-database/blob/main/advisories/github-reviewed/2021/12/GHSA-jfh8-c2jp-5v3q/GHSA-jfh8-c2jp-5v3q.json",
        },
      },
    ],
    references: [
  ...
  describe("getVulnGHSA", () => {
    ...
    it("should throw EntityNotFound when GHSA is not found", async () => {
      ...
      await expect(
        osvRepository.getVulnGHSA("GHSA-nonexistent"),
  ...
  describe("getVulnCVE", () => {
    ...
    it("should throw EntityNotFound when CVE is not found", async () => {
      ...
      await expect(
        osvRepository.getVulnCVE("CVE-2023-NONEXISTENT"),
      ).rejects.toThrow(EntityNotFound);
      expect(mockRepository.findOne).toHaveBeenCalledWith({
  ...
  describe("getVulnByCVEIDWithoutFailing", () => {
    it("should return OSV vulnerability when found", async () => {
      ...
      const result =
        await osvRepository.getVulnByCVEIDWithoutFailing("CVE-2021-44228");
    ...
    it("should handle special characters in CVE ID", async () => {
      ...
      const result =
        await osvRepository.getVulnByCVEIDWithoutFailing(specialCveId);
  ...
  describe("getVulnByOSVIDWithoutFailing", () => {
    it("should return OSV vulnerability when found", async () => {
      ...
      const result = await osvRepository.getVulnByOSVIDWithoutFailing(
        "GHSA-jfh8-c2jp-5v3q",
      );
    ...
    it("should return null when OSV ID is not found", async () => {
      ...
      const result =
        await osvRepository.getVulnByOSVIDWithoutFailing("GHSA-nonexistent");
  ...
  describe("Real-world OSV data scenarios", () => {
    it("should handle Log4Shell (GHSA-jfh8-c2jp-5v3q) complete data", async () => {
      ...
      expect(
        (result.affected as { package: { name: string } }[])[0]!.package.name,
      ).toBe("org.apache.logging.log4j:log4j-core");
    });
    ...
    it("should handle Python vulnerability (PYSEC)", async () => {
      ...
      const result =
        await osvRepository.getVulnByOSVIDWithoutFailing("PYSEC-2021-852");

      expect(result?.osv_id).toBe("PYSEC-2021-852");
      expect(
        (result?.affected as { package: { ecosystem: string } }[])[0]!.package
          .ecosystem,
      ).toBe("PyPI");
    });
    ...
    it("should handle Rust vulnerability (RUSTSEC)", async () => {
      ...
      const result =
        await osvRepository.getVulnByOSVIDWithoutFailing("RUSTSEC-2021-0145");
    ...
    it("should handle vulnerability with multiple affected packages", async () => {
      ...
      expect(affected[0]!.package.name).toBe(
        "org.apache.logging.log4j:log4j-core",
      );
      expect(affected[1]!.package.name).toBe(
  ...
  describe("Edge cases and error handling", () => {
    it("should handle undefined parameters gracefully", async () => {
      ...
      const result1 = await osvRepository.getVulnByCVEIDWithoutFailing(
        undefined as any,
      );
      const result2 = await osvRepository.getVulnByOSVIDWithoutFailing(
        undefined as any,
      );
    ...
    it("should handle null parameters gracefully", async () => {
      ...
      await expect(osvRepository.getVulnGHSA(null as any)).rejects.toThrow(
        EntityNotFound,
      );
      await expect(osvRepository.getVulnCVE(null as any)).rejects.toThrow(
        EntityNotFound,
      );
  ...
  describe("Service initialization", () => {
    ...
    it("should have all required methods", () => {
      ...
      expect(typeof osvRepository.getVulnByCVEIDWithoutFailing).toBe(
        "function",
      );
      expect(typeof osvRepository.getVulnByOSVIDWithoutFailing).toBe(
        "function",
      );
    });
  });
