...
export class TicketsController {
  ...
  @ApiOperation({
    summary: "Check if a ticket already exists for a vulnerability",
  })
  @ApiParam({ name: "org_id", description: "Organization ID" })
  ...
  async processAnalysisAutoResolve(
    ...
    const result =
      await this.ticketAutomationService.processCompletedAnalysis(analysis_id);
    return { data: result };
  }
  ...
  @Get("integrations")
  @ApiOperation({
    summary: "Get all configured integrations for the organization",
  })
  @ApiParam({ name: "org_id", description: "Organization ID" })
  @ApiResponse({
    status: 200,
  ...
  async getIntegrations(
    ...
    const configs =
      await this.ticketIntegrationService.getIntegrationConfigs(org_id);
  ...
  async configureClickUp(
    ...
    const config: ClickUpConfig = {
      auth_method:
        body.auth_method === "OAUTH"
          ? ClickUpAuthMethod.OAUTH
          : ClickUpAuthMethod.API_KEY,
  ...
  async removeIntegration(
    ...
    await this.ticketIntegrationService.deleteIntegrationConfig(
      org_id,
      provider,
    );
    return {};
  ...
  async testIntegration(
    ...
    const result = await this.ticketIntegrationService.testConnection(
      org_id,
      provider,
    );
    return { data: result };
  ...
  async toggleIntegration(
    ...
    await this.ticketIntegrationService.toggleIntegration(
      org_id,
      provider,
      enabled,
    );
  ...
  async getWorkspaces(
    ...
    const workspaces = await this.ticketIntegrationService.getWorkspaces(
      org_id,
      provider,
    );
  ...
  async getFolders(
    ...
    const folders = await this.ticketIntegrationService.getFolders(
      org_id,
      provider,
      space_id,
    );
  ...
  async getLists(
    ...
    const lists = await this.ticketIntegrationService.getLists(
      org_id,
      provider,
      parent_id,
    );
  ...
  async getExternalLinks(
    ...
    const links =
      await this.ticketIntegrationService.getExternalLinks(ticket_id);
  ...
  @ApiOperation({
    summary: "Bulk sync multiple tickets to an external provider",
  })
  @ApiParam({ name: "org_id", description: "Organization ID" })
  @ApiParam({
    name: "provider",
  ...
  @ApiResponse({
    status: 404,
    description: "Ticket or external link not found",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Bulk sync from external completed",
  })
