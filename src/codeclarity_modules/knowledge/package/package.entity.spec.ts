...
describe("Package Entity", () => {
  describe("Package entity properties", () => {
    ...
    it("should handle complex extra field structure", () => {
      ...
      expect((extra["maintainers"] as { name: string }[])[0]!.name).toBe(
        "John Doe",
      );
      expect((extra["repository"] as { directory: string }).directory).toBe(
        "packages/core",
      );
      expect((extra["bugs"] as { email: string }).email).toBe(
        "bugs@example.com",
      );
  ...
  describe("Version entity properties", () => {
    ...
    it("should handle complex dependency structures", () => {
      ...
      expect(
        (complexVersion.extra as Record<string, { react: string }>)[
          "peerDependencies"
        ]!.react,
      ).toBe(">=16.8.0");
      expect(complexVersion.extra["bundleDependencies"]).toContain(
        "internal-lib",
      );
  ...
  describe("Database column type compatibility", () => {
    ...
    it("should handle timestamptz column type", () => {
      ...
      expect(timestampPackage.time.toISOString()).toBe(
