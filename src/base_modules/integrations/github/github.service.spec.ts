...
describe('GithubIntegrationService', () => {
    ...
    describe('addGithubIntegration', () => {
        ...
        it('should successfully add a GitHub integration', async () => {
            organizationsRepository.getOrganizationById.mockResolvedValue(mockOrganization);
            ...
            organizationsRepository.saveOrganization.mockResolvedValue(mockOrganization);
            ...
            expect(organizationsRepository.getOrganizationById).toHaveBeenCalledWith(orgId, {
                integrations: true
            });
            expect(githubTokenService.getClassicTokenExpiryRemote).toHaveBeenCalledWith(
                linkGithubCreate.token
            );
            expect(githubTokenService.validateClassicTokenPermissions).toHaveBeenCalledWith(
                linkGithubCreate.token,
                {}
            );
        ...
        it('should successfully add a GitHub integration with expiry date', async () => {
            ...
            organizationsRepository.getOrganizationById.mockResolvedValue(orgWithoutIntegrations);
        ...
        it('should throw IntegrationWrongTokenType for wrong token type in request', async () => {
            ...
            await expect(
                service.addGithubIntegration(orgId, invalidRequest, mockAuthenticatedUser)
        ...
        it('should throw IntegrationWrongTokenType for wrong token prefix', async () => {
            ...
            await expect(
                service.addGithubIntegration(orgId, invalidTokenRequest, mockAuthenticatedUser)
            ).rejects.toThrow(IntegrationWrongTokenType);
        });
        ...
        it('should throw EntityNotFound when organization does not exist', async () => {
            organizationsRepository.getOrganizationById.mockResolvedValue(null as any);

            await expect(
                service.addGithubIntegration(orgId, linkGithubCreate, mockAuthenticatedUser)
    ...
    describe('modifyGithubIntegration', () => {
        ...
        it('should successfully modify a GitHub integration', async () => {
            ...
            expect(integrationsRepository.getIntegrationById).toHaveBeenCalledWith(integrationId);
            expect(integrationsRepository.saveIntegration).toHaveBeenCalled();
        });
    ...
    describe('getGithubIntegration', () => {
        ...
        it('should throw NotAuthorized when integration does not belong to organization', async () => {
            ...
            await expect(
                service.getGithubIntegration(orgId, integrationId, mockAuthenticatedUser)
            ).rejects.toThrow(NotAuthorized);
        });
    });
    ...
    describe('getToken', () => {
        ...
        it('should throw IntegrationTokenExpired and re-throw', async () => {
            ...
            await expect(service.getToken(integrationId)).rejects.toThrow(IntegrationTokenExpired);
        });
        ...
        it('should throw IntegrationInvalidToken and re-throw', async () => {
            ...
            await expect(service.getToken(integrationId)).rejects.toThrow(IntegrationInvalidToken);
        });
        ...
        it('should convert NotAMember to NotAuthorized', async () => {
            ...
            await expect(service.getToken(integrationId)).rejects.toThrow(NotAuthorized);
        ...
        it('should re-throw other errors', async () => {
            ...
            await expect(service.getToken(integrationId)).rejects.toThrow(customError);
        });
    });

    describe('getTokenTypeFromTokenString', () => {
        it('should return CLASSIC_TOKEN for ghp prefix', () => {
            const result = (service as any).getTokenTypeFromTokenString('ghp_test_token');
            expect(result).toBe(GithubTokenType.CLASSIC_TOKEN);
        });

        it('should return OAUTH_TOKEN for gho prefix', () => {
            const result = (service as any).getTokenTypeFromTokenString('gho_oauth_token');
